...
class Multi_features(nn.Module):
    def __init__(self, in_channels, out_channels, sensors):
        """
...
class Attention_Fusion_Net(nn.Module):
    def __init__(self, sc_ae_model, in_channel, out_channel, sensor, ration=1):
        ...
        self.M_Conv_layer = Multi_features(in_channel, out_channel, sensors=sensor)  # 获取多尺度的聚合特征
        # 特征矩阵聚拢
        self.layer_fusion0 = nn.Conv2d(in_channels=128, out_channels=64, kernel_size=(1, 1), bias=False)
        self.layer_relu0 = nn.ReLU()
        # 加入水平注意力机制
    ...
    def forward(self, x):
        ...
        return out_method1, out_method2, out_method3, out_method4   # 输出尺寸[8,64,1,106]
