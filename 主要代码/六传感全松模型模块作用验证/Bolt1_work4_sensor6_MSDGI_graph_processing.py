"""
    ...
    包括数据的标签和预处理过程
    二螺栓松动包含数据R61-R72；扭矩5/8/12/20共六种工况，只分析四种工况
    设置标签：0,1,2,3,
...
folder_path = 'D:/pyCharm_projects/数据集汇总/螺栓振动数据/振动_法兰_单螺栓松动四工况'
dpath = folder_path
...
def data_processing(path, batch_size, train_sample):
    ...
    data_CM[0][:] = R73_80[0][:]  # 扭矩2NM
    data_CM[1][:] = R73_80[1][:]
    data_CM[2][:] = R73_80[2][:]  # 5NM
    data_CM[3][:] = R73_80[3][:]
    data_CM[4][:] = R73_80[6][:]  # 12NM
    data_CM[5][:] = R73_80[7][:]
    data_CM[6][:] = R73_80[10][:]  # 20NM
    data_CM[7][:] = R73_80[11][:]
