"""
    主要是梁螺栓振动数据分析，数据级融合形成灰度图
    包括数据的标签和预处理过程
    双螺栓松动包含数据R1-R16；使用五2/4/6/10/20种工况分析  R1/R2
    设置标签：0,1,2,3,4
...
def data_processing(path, batch_size, train_sample):
    ...
    data_CM[0][:] = R1_18[0][:]  # 扭矩2NM
    data_CM[1][:] = R1_18[1][:]
    data_CM[2][:] = R1_18[2][:]  # 4NM
    data_CM[3][:] = R1_18[3][:]
    data_CM[4][:] = R1_18[4][:]  # 6NM
    data_CM[5][:] = R1_18[5][:]
    data_CM[6][:] = R1_18[8][:]  # 10NM
    data_CM[7][:] = R1_18[9][:]
    data_CM[8][:] = R1_18[14][:]  # 20NM
    data_CM[9][:] = R1_18[15][:]

    # 去除时间序列数据一号传感数据干扰量太大，所以剔除1(0,1)、6(10,11)号传感
    data = np.delete(data_CM, [0, 1, 2, 3, 4, 5, 6, 8, 10, 11, 12, 14, 15], 2)  # 8*327680*6
    ...
    def data_resample(data1, data2, data_len, data_step):
        ...
        :return:
        '''
        data_temp = np.zeros((126, 3, data_len), dtype=np.float64)  # 一个传感数划分
