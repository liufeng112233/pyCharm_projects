"""
    ...
    使用5/6/7/8/9/10/12/16/20九种工况分析  R1/R2
    设置标签：0,1,2,3,
...
def data_processing(path, batch_size, train_sample):
    ...
    data_CM = np.zeros([18, 327680, 16])
    data_CM[0][:] = R1_18[0][:]  # 扭矩5NM
    data_CM[1][:] = R1_18[1][:]

    data_CM[2][:] = R1_18[2][:]  # 6NM
    data_CM[3][:] = R1_18[3][:]

    data_CM[4][:] = R1_18[4][:]  # 7NM
    data_CM[5][:] = R1_18[5][:]

    data_CM[6][:] = R1_18[6][:]  # 8NM
    data_CM[7][:] = R1_18[7][:]

    data_CM[8][:] = R1_18[8][:]  # 9NM
    data_CM[9][:] = R1_18[9][:]

    data_CM[10][:] = R1_18[10][:]  # 10NM
    data_CM[11][:] = R1_18[11][:]

    data_CM[12][:] = R1_18[12][:]  # 12NM
    data_CM[13][:] = R1_18[13][:]

    data_CM[14][:] = R1_18[14][:]  # 16NM
    data_CM[15][:] = R1_18[15][:]

    data_CM[16][:] = R1_18[16][:]  # 20NM
    data_CM[17][:] = R1_18[17][:]

    # 去除时间序列数据一号传感数据干扰量太大，所以剔除1(0,1)、6(10,11)号传感
    ...
    data_work6 = data_resample(data[10][:], data[11][:], data_len, data_step)
    data_work7 = data_resample(data[12][:], data[13][:], data_len, data_step)
    data_work8 = data_resample(data[14][:], data[15][:], data_len, data_step)
    data_work9 = data_resample(data[16][:], data[17][:], data_len, data_step)

    def processing(data, labels, sample):
        # Code_data=124*10240
        train_data = data[0:16][:, :]  # 96
        test_data = data[94:126][:, :]  # 28

        train_labels = np.array([labels for i in range(0, 16)])
    ...
    tr_x6, te_x6, tr_y6, te_y6 = processing(data_work6, 5, train_sample)
    tr_x7, te_x7, tr_y7, te_y7 = processing(data_work7, 6, train_sample)
    tr_x8, te_x8, tr_y8, te_y8 = processing(data_work8, 7, train_sample)
    tr_x9, te_x9, tr_y9, te_y9 = processing(data_work9, 8, train_sample)
    ...
    train_data = train_data8  # (96*num)*8*10240
    train_label = lab_train8
    test_data = test_data8  # (32*num)*10240
