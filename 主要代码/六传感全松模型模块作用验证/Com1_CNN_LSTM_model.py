...
class Channel_Fusion(nn.Module):
    def __init__(self, sensors):
        ...
        self.layer_fusion1 = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=16, kernel_size=(sensors, 5), stride=(1, 8)),
            nn.ReLU(),
...
class LSTM_block(nn.Module):
    def __init__(self):
        ...
        self.layer1 = nn.LSTM(input_size=128 * 80, hidden_size=256, bidirectional=True, batch_first=True)

    def forward(self, x):
        [B,C,S,L] = x.size()
        x_temp = x.permute(0, 2, 1, 3).reshape(B, S, C * L)
