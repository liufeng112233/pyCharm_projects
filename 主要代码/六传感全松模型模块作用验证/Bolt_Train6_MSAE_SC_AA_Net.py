...
def train_test_Net(model, learning_rate, total_epochs, num_classes, batch_size, train_num, sensor_num, train_loader, test_loader):
    # 更新学习率
    ...
    for epoch in range(total_epochs):
        ...
        for step, (x, label) in enumerate(train_loader):
            ...
            x1 = x.view(batch_size, 1, sensor_num, 10240)
            output_train, _, _ = model(x1)
            ...
            if step % 5 == 0:
                print('Epoch :', epoch, '|', 'train_loss:%.8f' % loss.data, '|', 'lr :', curr_lr)
            # 保存给个batch的损失数值
        ...
        with torch.no_grad():
            for test_idx, (x_test, label_test) in enumerate(test_loader):
                ...
                x_test1 = x_test.view(batch_size, 1, sensor_num, 10240)
                output_test, test_hidden_features, _ = model(x_test1)
            ...
            # 验证集平均损失D:\pyCharm_projects\主要代码\六传感全松模型模块作用验证\轴注意力自校正权重融合分类模型
            test_epochs_avgloss.append(np.average(test_epoch_loss))
    MSAE_PATH_name = "D:/pyCharm_projects/主要代码/六传感全松模型模块作用验证/激活函数影响模型/分类模型/GELU_SCAEAA_heng_train{}_" \
                     "work{}_epoch{}_lr{}.pt". \
        format(train_num, num_classes, total_epochs, learning_rate)
    ...
    with torch.no_grad():
        for step2, (data2, label2) in enumerate(test_loader):
            ...
            x_data21 = x_data2.view(batch_size, 1, sensor_num, 10240)
            _, Hidden_Features_test, weight = model(x_data21)
    ...
    return np.array(train_epochs_avgloss), np.array(test_epochs_avgloss), \
           np.array(test_accuracy), out_feature, out_label, weight
