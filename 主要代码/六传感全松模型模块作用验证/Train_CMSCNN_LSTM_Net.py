...
def train_test_Net(model, learning_rate, total_epochs, num_classes, train_sample, train_loader, test_loader):
    # 更新学习率
    ...
    for epoch in range(total_epochs):
        ...
        with torch.no_grad():
            for test_idx, (x_test, label_test) in enumerate(test_loader):
                ...
                output_test1, test_hidden_features = model(x_test1)
                loss = loss_fun(output_test1, label_test)
                # 损失值
                test_epoch_loss.append(loss.item())
                # 准确率
                predicted = output_test1.argmax(dim=1)
    ...
    MSAE_PATH_name = "D:/pyCharm_projects/主要代码/六传感全松模型模块作用验证/对比模型/CMSCNN_LSTM/bolt1_train{}_work{}_epoch{}_lr{}.pt". \
        format(train_sample, num_classes, total_epochs, learning_rate)
