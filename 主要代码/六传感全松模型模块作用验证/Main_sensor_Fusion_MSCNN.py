...
from multi_Sensor_Fusion_MSCNN_model import MSCNN_sensor_fusion_model
import My_TSNE_sensor6
from torch import nn
import torch.nn.functional as F
from Bolt_Train_sensor_Fusion_MSCNN_Net import train_test_Net
...
if __name__ == '__main__':
    ...
    train_loader_shuffle, test_loader_shuffle, train_loader, test_loader \
        = Bolt_work9_sensor6_MSDGI_graph_processing.data_processing(path, batch_size)

    MSCNN = MSCNN_sensor_fusion_model(num_classes=num_classes).to(devices)
    ...
    plt.xlabel("EPOCH")
    plt.legend(loc=2)
    plt.savefig('E:/桌面/训练数据记录/MSSCAE六传感图片记录/MSSCAE_work{}_epoch{}_lr{}.jpg'.format(num_classes, Total_Epochs, learning_rate),
                dpi=1200)
    plt.show()
