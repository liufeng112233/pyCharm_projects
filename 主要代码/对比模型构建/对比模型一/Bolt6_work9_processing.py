...
def data_processing(path, batch_size):
    dpath = path
    ...
    def data_resample(data1, data2, data_len, data_step):
        ...
        data_temp = np.zeros((int(2 * (N + 1)), 6, data_len), dtype=np.float64)  # 一个传感数划分
        for i in range(N + 1):
    ...
    def processing(data, labels):
        # Code_data=124*10240
        train_data = data[0:280][:, :]  # 96
        test_data = data[280:320][:, :]  # 28

        train_labels = np.array([labels for i in range(0, 280)])
