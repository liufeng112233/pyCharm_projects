...
folder_DIRG = 'D:/pyCharm_projects/数据集汇总/DIRG轴承数据/同速100同负载000不同深度'
dpath = folder_DIRG
...
def data_processing(dpath, batch_size):
    ...
    def bear_data_load(dpath):
        ...
        Bear_data = np.zeros([7, 512000, 6])
        data_length = np.zeros((7, 1))
        for i in range(7):
            data_DE = SC.loadmat(os.path.join(dpath, files[i]))
            name_key = list(data_DE)
            Bear_data[i][:] = data_DE[name_key[3]]
            # data_length[i] = Len
        return Bear_data
    ...
    def processing(data, labels):
        # Code_data=124*10240
        train_data = data[0:200][:, :]  # 200
        test_data = data[200:240][:, :]  # 40

        train_labels = np.array([labels for i in range(0, 200)])
    ...
    train_data = train_data6  # 800*2*2048
    train_label = lab_train6
    test_data = test_data6  # 360*2*2048
