"""
    ...
    包括数据的标签和预处理过程
    4螺栓松动包含数据R73-R80；扭矩5/8/12/20共九种工况
    使用5/8/12/20四种工况分析  R73/R74
    设置标签：0,1,2,3,
...
folder_path = 'D:/pyCharm_projects/数据集汇总/螺栓振动数据/振动_法兰_四螺栓松动四工况'
dpath = folder_path
class_number = 4  # 说明数据分为四类
...
def data_processing(dpath, batch_size):
    ...
    # 获取四类数据8传感
    R73_80 = beam_load(dpath)
    data_CM = np.zeros([8, 327680, 16])
    data_CM[0][:] = R73_80[0][:]  # 扭矩5NM
    data_CM[1][:] = R73_80[1][:]
    data_CM[2][:] = R73_80[2][:]  # 8NM
    data_CM[3][:] = R73_80[3][:]
    data_CM[4][:] = R73_80[4][:]  # 12NM
    data_CM[5][:] = R73_80[5][:]
    data_CM[6][:] = R73_80[6][:]  # 20NM
    data_CM[7][:] = R73_80[7][:]

    # 去除时间序列数据一号传感数据干扰量太大，所以剔除
    ...
    train_data = train_data3  # 384*8*10240
    train_label = lab_train3
    test_data = test_data3  # 112*8*10240
    test_label = lab_test3
