"""
    ...
    采样12KHz、驱动端轴承故障，传感1：驱动端数据；传感2：风扇端数据，传感3：基座数据
    先共同负载情况下，三种故障深度、三种故障类型、标准工况（无）
    共计9种工况
"""
import numpy as np
import os
...
folder_path = 'D:/pyCharm_projects/数据集汇总/西储轴承数据12K/负载零不同故障'
# folder_path = 'D:/pyCharm_projects/数据集汇总/西储轴承数据12K/负载一不同故障'
# folder_path = 'D:/pyCharm_projects/数据集汇总/西储轴承数据12K/负载二不同故障'
# folder_path = 'D:/pyCharm_projects/数据集汇总/西储轴承数据12K/负载三不同故障'
dpath = folder_path
...
def data_processing(dpath, batch_size):
    ...
    def bear_data_load(dpath):
        ...
        Bear_data = np.zeros([9, 500000, 3])
        data_length = np.zeros((9, 1))
        for i in range(9):
            data_DE = SC.loadmat(os.path.join(dpath, files[i]))
            name_key = list(data_DE)
            [Len, _] = data_DE[name_key[3]].shape
            Bear_data[i][0:Len, :] = np.hstack((data_DE[name_key[3]], data_DE[name_key[4]], data_DE[name_key[5]]))
            data_length[i] = Len
    ...
    def data_resample(data, data_length_min, data_len, data_step):
        ...
        data_temp = np.zeros((N + 1, 3, data_len), dtype=np.float64)  # 一个传感数划分
        for i in range(N + 1):
