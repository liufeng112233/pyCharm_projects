...
# folder_path_bear20 = 'D:/pyCharm_projects/数据集汇总/东南轴承齿轮数据/轴承数据五工况/负载20'
# folder_path_bear30 = 'D:/pyCharm_projects/数据集汇总/东南轴承齿轮数据/轴承数据五工况/负载30'
# folder_path_gea20 = 'D:/pyCharm_projects/数据集汇总/东南轴承齿轮数据/齿轮箱五工况/负载20'
# folder_path_gear30 = 'D:/pyCharm_projects/数据集汇总/东南轴承齿轮数据/齿轮箱五工况/负载30'
...
def data_processing(dpath, batch_size):
    ...
    def bear_data_load(dpath):
        ...
        Bear_data = np.zeros([5, 1048560, 6])
        data_length = np.zeros((5, 1))
        for i in range(5):
    ...
    data_work1 = data_resample(Bear_data[2][:], data_point_number_min, data_len, data_step)
    data_work2 = data_resample(Bear_data[3][:], data_point_number_min, data_len, data_step)
    data_work3 = data_resample(Bear_data[0][:], data_point_number_min, data_len, data_step)
    data_work4 = data_resample(Bear_data[4][:], data_point_number_min, data_len, data_step)
    data_work5 = data_resample(Bear_data[1][:], data_point_number_min, data_len, data_step)
    ...
    def processing(data, labels):
        # Code_data=124*10240
        train_data = data[0:320][:, :]  # 80
        test_data = data[320:400][:, :]  # 36

        train_labels = np.array([labels for i in range(0, 320)])
        test_labels = np.array([labels for i in range(0, 80)])
    ...
    train_data = train_data4  # 800*2*2048
    train_label = lab_train4
    test_data = test_data4  # 360*2*2048
