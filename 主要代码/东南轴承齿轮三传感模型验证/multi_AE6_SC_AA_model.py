...
class Attention_Fusion_Net(nn.Module):
    def __init__(self, sc_ae_model, in_channel, out_channel, ration=1):
        """
        ...
        self.M_Conv_layer = Multi_features(in_channel, out_channel)  # 获取多尺度的聚合特征
        # 特征矩阵聚拢
        self.layer_fusion0 = nn.Conv2d(in_channels=128, out_channels=64, kernel_size=(1, 1), bias=False)
        self.layer_relu0 = nn.Mish()
        # 加入水平注意力机制
        self.Axis_Horizontal_layer = Axis_Horizontal_Attention_block(64, ration)
        # 加入水平注意力机制
        self.Axis_Portrait_layer = Axis_Portrait_Attention_block(64, ration)
