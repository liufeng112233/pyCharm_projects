...
class Axis_Horizontal_Attention_block(nn.Module):
    def __init__(self, channel, ration=1):
        """
...
class Axis_Portrait_Attention_block(nn.Module):
    ...
    def forward(self, x):
        ...
        ration = self.ration
        # Qh
        x_qw1 = self.relu_FC_w1(self.layer_Qw(x))  # bxC/rxHxW
        x_Qw = x_qw1.view(batch, Width, -1)  # bxWx(H*C/r)
        # Kh
        x_Kw1 = self.relu_FC_w2(self.layer_Kw(x))  # bxC/rxHxW
        x_Kw = x_Kw1.view(batch, Width, -1).transpose(1, 2)  # bx(H*C/r)xW

        # Vh
        x_Vw1 = self.relu_FC_w3(self.layer_Vw(x))
