...
folder_path = 'D:/pyCharm_projects/主要代码/数据/振动_法兰_六螺栓松动'
...
def data_processing(path, batch_size):
    ...
    data_work2 = np.vstack(data_resample(data[6, :], data[7, :], data_len, data_step)).reshape(126, 1, 10240)  # 8NM
    data_work3 = np.vstack(data_resample(data[12, :], data[13, :], data_len, data_step)).reshape(126, 1, 10240)  # 12NM
    data_work4 = np.vstack(data_resample(data[16, :], data[17, :], data_len, data_step)).reshape(126, 1, 10240)  # 20NM
    ...
    train_data = train_data3  # 384*1*10240
    train_label = lab_train3
    test_data = test_data3  # 128*1*10240
