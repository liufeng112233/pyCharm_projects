"""
    ...
    使用5/8/10/12/16/20l六种工况分析  R1/R2
    设置标签：0,1,2,3,4，5
...
def data_processing(path, batch_size, sensor):
    dpath = path
    ...
    sensor = sensor  # 传感号码
    data = data_select(R1_12, sensor)  # 4号传感9种工况18条数据
    ...
    data_work2 = np.vstack(data_resample(data[2, :], data[3, :], data_len, data_step)).reshape(126, 1, 10240)  # 8NM
    data_work3 = np.vstack(data_resample(data[4, :], data[5, :], data_len, data_step)).reshape(126, 1, 10240)  # 10NM
    data_work4 = np.vstack(data_resample(data[6, :], data[7, :], data_len, data_step)).reshape(126, 1, 10240)  # 12NM
    data_work5 = np.vstack(data_resample(data[8, :], data[9, :], data_len, data_step)).reshape(126, 1, 10240)  # 16NM
    data_work6 = np.vstack(data_resample(data[10, :], data[11, :], data_len, data_step)).reshape(126, 1, 10240)  # 20NM

    def processing(data, labels):
        # Code_data=124*10240
        train_data = data[0:64][:, :]  # 96
        test_data = data[70:126][:, :]  # 32

        train_labels = np.array([labels for i in range(0, 64)])
    ...
    train_data = train_data5  # 384*1*10240
    train_label = lab_train5
    test_data = test_data5  # 128*1*10240
    test_label = lab_test5
