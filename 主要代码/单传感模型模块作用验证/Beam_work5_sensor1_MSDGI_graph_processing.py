...
folder_path = 'D:/pyCharm_projects/数据集汇总/螺栓振动数据/振动_横梁_双螺栓松动'
...
def data_processing(path, batch_size, sensors_order):
    ...
    data_work2 = np.vstack(data_resample(data[2, :], data[3, :], data_len, data_step)).reshape(126, 1, 10240)  # 7NM
    data_work3 = np.vstack(data_resample(data[6, :], data[7, :], data_len, data_step)).reshape(126, 1, 10240)  # 9NM
    data_work4 = np.vstack(data_resample(data[10, :], data[11, :], data_len, data_step)).reshape(126, 1, 10240)  # 12NM
    data_work5 = np.vstack(data_resample(data[14, :], data[15, :], data_len, data_step)).reshape(126, 1, 10240)  # 20NM
    ...
    train_data = train_data4  # 384*1*10240
    train_label = lab_train4
    test_data = test_data4  # 128*1*10240
