...
# folder_path = 'D:/pyCharm_projects/主要代码/数据/振动_法兰_六螺栓松动_横向'
# data_save_path = 'D:/pyCharm_projects/主要代码/预处理数据保存'
# dpath = folder_path
# class_number = 6  # 说明数据分为四类
...
def data_processing(path, batch_size, sensor):
    ...
    data_work4 = np.vstack(data_resample(data[8, :], data[9, :], data_len, data_step)).reshape(126, 1, 10240)  # 12NM
    data_work5 = np.vstack(data_resample(data[12, :], data[13, :], data_len, data_step)).reshape(126, 1, 10240)  # 16NM
    data_work6 = np.vstack(data_resample(data[16, :], data[17, :], data_len, data_step)).reshape(126, 1, 10240)  # 20NM
