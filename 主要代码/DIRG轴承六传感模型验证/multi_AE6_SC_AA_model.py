...
import numpy as n
from Axis_attention_sensor6_block import Axis_Portrait_Attention_block, Axis_Horizontal_Attention_block
...
class Multi_features(nn.Module):
    def __init__(self, in_channels, out_channels, activate_F):
        ...
        # 第一层卷积
        self.features1 = nn.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=(6, 16),
                                   stride=(1, 16), padding=(0, 0))
        self.features2 = nn.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=(6, 32),
                                   stride=(1, 16), padding=(0, 8))
        self.features3 = nn.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=(6, 64),
                                   stride=(1, 16), padding=(0, 24))
        self.features4 = nn.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=(6, 128),
...
class Attention_Fusion_Net(nn.Module):
    def __init__(self, sc_ae_model, in_channel, out_channel, activate_F, ration=1):
        """
        ...
        self.M_Conv_layer = Multi_features(in_channel, out_channel, activate_F)  # 获取多尺度的聚合特征
        # 特征矩阵聚拢
        self.layer_fusion0 = nn.Conv2d(in_channels=128, out_channels=64, kernel_size=(1, 1), bias=False)
        self.layer_relu0 = activate_F
        # 加入水平注意力机制
        self.Axis_Horizontal_layer = Axis_Horizontal_Attention_block(64, activate_F, ration)
        # 加入水平注意力机制
        self.Axis_Portrait_layer = Axis_Portrait_Attention_block(64, activate_F, ration)
