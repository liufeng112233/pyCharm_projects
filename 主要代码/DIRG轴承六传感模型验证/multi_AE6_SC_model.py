...
'''
    输入：多传感构成的灰度图batch*1*8*5128 MSDGI
    编码：对数据级的融合数据通过多尺度卷积
    解码：反卷积，最后采用多层线性层解码8个(batch*1*10240)，最后对解码融合batch*1*8*5128
'''
...
class MySC_AE(nn.Module):
    def __init__(self):
        ...
        # b*32*2*318
        self.de_layer2 = nn.ConvTranspose1d(in_channels=32, out_channels=8, kernel_size=(3, 3), stride=(1, 3))
        self.de_fc2 = nn.ReLU()
        self.de_SC_Net2 = SC_Net(in_channel=8)
        # 8*64*3*954
        self.de_layer3 = nn.ConvTranspose1d(in_channels=8, out_channels=1, kernel_size=(2, 3), stride=(2, 3))
        self.de_fc3 = nn.ReLU()
        # 8*1*6*2862
    ...
    def data_cut(self, features1):
        ...
        '''
        # features1 = torch.randn(2, 1, 6, 2826)
        ...
        # NO.5
        temp5 = features1[0][0][4, :]
        for i in range(b - 1):
            x5 = torch.cat([temp5, features1[i + 1][0][4, :]], dim=0)
            temp5 = x5
        x5 = x5.view(b, L1)

        # NO.6
        temp6 = features1[0][0][5, :]
        for i in range(b - 1):
            x6 = torch.cat([temp6, features1[i + 1][0][5, :]], dim=0)
            temp6 = x6
        x6 = x6.view(b, L1)
